...
# must download
run_job nmap -Pn -sV --script "dns-nsid" ${host} -p"${port}" -oA "${dest_dir}/banner_p${port}" 
run_job nmap -Pn --script "(default and *dns*) or fcrdns or dns-srv-enum or dns-random-txid or dns-random-srcport" ${host} -p"${port}" -oA "${dest_dir}/enum_p${port}"
