...
# must download
run_job nmap -Pn -sU --script ipmi-version -p "${port}" "${host}" -oA "${dest_dir}/version_p${port}"
run_job ipmitool -I lanplus -C 0 -H "${host}" -p "${port}" -U root -P root user list > "${dest_dir}/cipher_0_p${port}" 
run_job ipmitool -I lanplus -H "${host}" -p "${port}" -U '' -P '' user list > "${dest_dir}/anon_login_p${port}" 
