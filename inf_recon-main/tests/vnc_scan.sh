...
run_job nmap -Pn -p"${port}" -sV --script vnc-info,vnc-title "${host}" -oA "${dest_dir}/general_p${port}"
run_job nmap -Pn -p"${port}" -sV --script realvnc-auth-bypass "${host}" -oA "${dest_dir}/realvnc_auth_bypass_p${port}"
run_job nmap -Pn -p"${port}" -sV --script vnc-brute "${host}" -oA "${dest_dir}/brute_p${port}"
